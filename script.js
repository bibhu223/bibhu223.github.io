...
(() => {
  ...
  function draw(now){const t=now-start;ctx.clearRect(0,0,w,h);grid();particlesDraw(t);const p=(t*.000035)%1,x=w*(.58+p*.24),y=h*.69-Math.sin(p*6.283)*24-p*72;path(x,y,t);field(x,y,t);lidar(x,y,t);robot(x,y,t);raf=requestAnimationFrame(draw)}
